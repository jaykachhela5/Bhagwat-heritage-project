...
export const booksApi = {
  ...
  add: (payload: Omit<Book, "_id" | "createdAt">) =>
    apiClient.post<{ message: string; book: Book }>("/api/books", payload),
  update: (id: string, payload: Partial<Omit<Book, "_id" | "createdAt">>) =>
    apiClient.put<{ message: string; book: Book }>(`/api/books/${id}`, payload),
...
export const issuesApi = {
  ...
  update: (id: string, payload: { status?: "Issued" | "Returned"; returnDate?: string }) =>
    apiClient.put<{ message: string; issue: Issue }>(`/api/issue/${id}`, payload),
};
