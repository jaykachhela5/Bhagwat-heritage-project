...
export const StatStrip = memo(function StatStrip({ items, variant = "default" }: StatStripProps) {
  const rootClass =
    variant === "gauseva"
      ? "rounded-3xl border border-white/10 bg-[#0d6179] p-5 md:p-7 shadow-[0_14px_30px_rgba(0,0,0,0.18)]"
  ...
  const cardClass =
    variant === "gauseva"
      ? "rounded-xl border border-white/10 bg-[#0c5871] p-4 text-center"
  ...
  const labelClass = variant === "gauseva" ? "text-xs md:text-sm text-[#dce7ec]" : "text-xs md:text-sm text-[#7a5634]";
