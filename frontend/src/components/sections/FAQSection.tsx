...
export const FAQSection = memo(function FAQSection({ title, items, variant = "default" }: FAQSectionProps) {
  const rootClass =
    variant === "gauseva"
      ? "rounded-3xl border border-white/10 bg-[#0d6179] p-6 md:p-8 shadow-[0_16px_35px_rgba(0,0,0,0.18)]"
      : "rounded-3xl border border-[#dce8f4] bg-white p-6 md:p-8 shadow-sm";

  const titleClass =
    variant === "gauseva"
      ? "text-[24px] font-semibold uppercase tracking-[0.18em] text-[#ef9a1e] mb-4"
      : "text-2xl md:text-3xl font-black text-[#0c5b7d] mb-4";

  const detailsClass =
    variant === "gauseva"
      ? "rounded-xl border border-white/10 bg-[#0c5871] p-4"
  ...
  const answerClass = variant === "gauseva" ? "mt-2 text-sm leading-6 text-[#dce7ec]" : "mt-2 text-sm text-[#4f6272]";
